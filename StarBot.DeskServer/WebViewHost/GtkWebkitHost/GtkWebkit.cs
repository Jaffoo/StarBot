...
using StarBot.DeskServer.WebViewHost;
using StarBot.DeskServer;
using Microsoft.Web.WebView2.Core;
using Newtonsoft.Json.Linq;

namespace StarBot.DeskServer.WebViewHost.GtkWebkitHost
{
    ...
    internal class GtkWebkit : IWebView
    {
        ...
        public async Task Initialization(IWindow window, int width, int height, string url, Type blazorComponent, string blazorSelector)
        {
            ...
            // };
            var conf = JObject.Parse(File.ReadAllText("appsettings.json"));
            var post = conf["Urls"].ToString().Replace("http://*:", "");
            GtkWebkitApi.webkit_web_view_run_javascript(Handle, $"sessionStorage.setItem('HttpPort',{post})", IntPtr.Zero, IntPtr.Zero, IntPtr.Zero);
            if (IsDebug == true)
            {
