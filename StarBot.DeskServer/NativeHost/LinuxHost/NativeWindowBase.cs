    ...
    internal class NativeWindowBase
    {
        ...
        protected virtual bool OnMotionNotifyEvent(IntPtr widget, IntPtr ev, IntPtr data)
        {
            ...
            else
            {
                if (!CanReSize)
                    return false;
                var left = Math.Abs(eventMotion.X - allocation.X) <= 10;
                var top = Math.Abs(eventMotion.Y - allocation.Y) <= 10;
                var right = Math.Abs(eventMotion.X - allocation.Width) <= 10;
                var bottom = Math.Abs(eventMotion.Y - allocation.Height) <= 10;
        ...
        protected void RegisterHandel(IntPtr handle, string name, GtkWidgetEventDelegate @event)
        {
            GtkApi.g_signal_connect_data(handle, name,
                Marshal.GetFunctionPointerForDelegate(@event), IntPtr.Zero, IntPtr.Zero, 0);
        }
