<template>
    <el-select v-model="selected" filterable remote :multiple="multiple" :remote-method="searchGroup" :loading="loading"
        placeholder="请选择群" @change="(data: any) => emit('change', data.toString())">
        <el-option v-for="item in groups" :key="item.group_id" :label="item.group_name" :value="item.group_id">
        </el-option>
    </el-select>
</template>
...
const searchGroup = async (keywords = "") => {
